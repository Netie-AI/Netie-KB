...
- **W-0005** `active` `high` - Cross-machine agent sync - GitHub bus, registry, vault (sync, containers, github, openvault)
...
- **F-0026** `active` `high` - The same six skills exist twice with different content and no authoritative copy (skills, drift, r-0016, cursor, crew)
- **F-0042** `active` `high` - Estate law tree D:\Netie has zero commits and docs cite a detached E: drive (git, R-0008, paths, drive-detach, F-0029)
...
- **F-0023** `active` `medium` - Distilled laptop-use knowledge sat unpromoted in a capture while lanes re-derived (distill, promotion, skills)
- **F-0024** `unverified` `medium` - AirGPT merge wave: containment proofs beat per-PR CI archaeology (merge-wave, git-containment, kilo-billing, ci, airgpt)
- **F-0025** `active` `medium` - Two agent lanes on one ticket queue duplicate work unless the ticket is claimed first (multi-agent, orchestration, collision, tickets)

## Skills

- **S-0001** `active` `high` - Coordinate as one fleet - presence, locks, queue (coordination, presence, claims, a2a)
- **S-0002** `active` `high` - Drive the laptop reliably - UACC and Playwright routing (laptop-use, uacc, playwright, computer-control)
- **S-0004** `active` `high` - Find a skill - the estate map of where skills live and which copy wins (skills, registry, mcp, discovery, r-0016)
- **S-0003** `active` `medium` - Pull, use, and improve skills from the registry (skills, registry, mcp, learning)
